/**
 * data.js — Carga JSON y renderiza contenido dinámicamente.
 *
 * Separa el contenido del markup. Para actualizar un miembro,
 * proyecto o publicación solo se edita el JSON correspondiente.
 */
...
export async function loadAllData() {
  try {
    const [researchLines, members, projects, publications] = await Promise.all([
      fetch('data/research-lines.json').then(r => r.json()),
    ...
    updateCounters(members, projects, publications);

  } catch (err) {
    console.error('[SemiGITA] Error cargando datos:', err);
  }
}
  ...
  active.forEach((member, i) => {
    ...
    card.innerHTML = `
      <div class="member-card-inner">
        <div class="member-card-front" aria-hidden="false">
        ...
        <div class="member-card-back" aria-hidden="true">
          <p class="member-name">${member.name}</p>
...
function renderPublications(publications) {
  ...
  years.forEach(year => {
    ...
    byYear[year].forEach(pub => {
      pubsList.appendChild(buildPublicationItem(pub));
    });
...
function buildPublicationItem(pub) {
  ...
  const doiLink = pub.doi
    ? `<a href="https://doi.org/${pub.doi}" class="badge badge-cyan link-accent" target="_blank" rel="noopener noreferrer">DOI</a>`
    : '';
  const urlLink = pub.url && !pub.doi
    ? `<a href="${pub.url}" class="badge badge-cyan link-accent" target="_blank" rel="noopener noreferrer">Ver →</a>`
    : '';
...
// ── CONTADORES EN HERO / ABOUT ─────────────────────────────────
function updateCounters(members, projects, publications) {
  const set = (id, val) => {
